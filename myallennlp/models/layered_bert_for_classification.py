...
@Model.register("layered_bert_for_classification")
class LayeredBertForClassification(Model):
    """
    ...
    scaling_temperature: ``int``, optional (default: 1)
        Scaling temperature parameter for better calibration
...
#        self.bert_model.requires_grad = trainable
        ...
        self.num_predicted_hidden_layers = num_predicted_hidden_layers
        self.scaling_temperature = scaling_temperature
        self._sum_weights = torch.nn.Parameter(torch.randn(self._layer_index + 1))
        initializer(self._classification_layer)

        self._sum_weights = torch.nn.Parameter(torch.nn.functional.normalize(self._sum_weights, p=2, dim=0))
    ...
    def forward(self,  # type: ignore
        ...
        _, pooled = self.bert_model(input_ids=input_ids,
                                    ...
                                    output_all_encoded_layers=True, layer_index=self._layer_index, num_predicted_hidden_layers=self.num_predicted_hidden_layers 
                                    )

        pooled = self._dropout(pooled)

#        pooled = self._sum_weights.expand(pooled.size()[2], pooled.size()[1], pooled.size()[0]).transpose(0,2) * pooled
#        pooled = torch.sum(pooled, 0)
        pooled = torch.einsum("a,abc->bc", (self._sum_weights, pooled))

        # apply classification layer
        logits = self._classification_layer(pooled)/self.scaling_temperature 
